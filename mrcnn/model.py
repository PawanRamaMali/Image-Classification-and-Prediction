...
class MaskRCNN(object):
    ...
    def save_config(self, model_path, train_dataset):
        config_dict = self.config.to_dict()
        
        myclass_names = dict(zip(train_dataset.class_ids, train_dataset.class_names))
        myclass_names = {int(k):v for k,v in myclass_names.items()}
            
        config_dict["class_names"] = myclass_names
        
#        model_name = model_path.split('\\')[-1]
        model_name = os.path.basename(model_path)
        file_name = model_name + '_config.json'
        file_path = os.path.join(model_path,file_name)

        with open(file_path , 'w') as fp:
            json.dump(config_dict, fp, default=self.myconverter)
    ...
    def train(self, train_dataset, val_dataset, learning_rate, epochs, layers,
        ...
        # Create log_dir if it does not exist and save config parameters
        if not os.path.exists(self.log_dir):
            os.makedirs(self.log_dir)
            self.save_config(self.log_dir, train_dataset)
